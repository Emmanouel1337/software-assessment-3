...
def getAsc(steamid):
    conn = sqlite3.connect(f'{steamid}_fulldatabase.db')
    cursor = conn.cursor()
...
def getBeaten(steamid):
    conn3 = sqlite3.connect(f'{steamid}_fulldatabase.db')
...
def getMostPlayed(steamid):
    conn4 = sqlite3.connect(f'{steamid}_fulldatabase.db')
    cursor4 = conn4.cursor()
    cursor4.execute(f'''SELECT * FROM "{steamid}_fulldatabase" WHERE playtime_forever IS NOT NULL ORDER BY playtime_forever DESC LIMIT 8;''')
